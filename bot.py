...
class MyClient(discord.Client):
    ...
    async def on_message(self, message):
        ...
        if message.content.lower() == "nez":
            if (
                day == self.lastday
                and month == self.lastmonth
                and h == self.lasthour
                and m == self.lastmin
            ):
                ...
                await message.add_reaction("❌")
                await message.reply(
                    "Aie aie aie quelqu'un t'a volé tes zapatos :sob:"
                )
                ...
                await message.add_reaction("👃")
                await message.add_reaction("❌")
                await message.reply("Presque mais trop tôt ! -1 loser")
                if message.author.name in self.leaderb:
                ...
                await message.add_reaction("❌")
                await message.reply(
                    "T'es complètement bourré ou quoi? :rofl: :call_me:"
                )

        if message.content.lower() == "nezsec":
            d = str(60 - s)
            await message.reply(
                "Il reste " + d + " secondes avant la prochaine minute"
            )

        if message.content.lower() == "leaderb":
            embed = discord.Embed(
                title="Sheesherboard", color=message.author.color
            )

            self.leaderb = {
                k: v
                for k, v in sorted(
                    self.leaderb.items(), key=lambda item: item[1], reverse=True
                )
            }
            s = ""
            for name, score in self.leaderb.items():
                s += f"**{name}** a **{str(score)}** points de nez\r\n"
